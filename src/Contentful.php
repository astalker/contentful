...
class Contentful
{
    ...
    private function doRequest($spaceData, $spaceName, $endpointUrl, $exceptionMessage, $api, $queryType = null, $cacheDisambiguator = '', array $parameters, array $options)
    {
        ...
        if ($api !== self::CONTENT_MANAGEMENT_API && $cacheItem->isHit()) {
            ...
            if (is_string($cacheItemJson) && strlen($cacheItemJson) > 0) {
                ...
                return $this->buildResponseFromRaw(json_decode($cacheItemJson, $assoc = true), $spaceData['name'], null, $this->pluckLocaleFromParameters($parameters));
            }
        }
        ...
        try {
            ...
            if ($api === self::CONTENT_DELIVERY_API && $fallbackCacheItem->isHit()) {
                ...
                if (is_string($fallbackJson) && strlen($fallbackJson) > 0) {
                    ...
                    return $this->buildResponseFromRaw(json_decode($fallbackJson, $assoc = true), $spaceData['name'], null, $this->pluckLocaleFromParameters($parameters));
                }
            }
        ...
        return $this->buildResponseFromRaw($response->json(), $spaceData['name'], $assetDecorator, $this->pluckLocaleFromParameters($parameters));
    }
     ...
     * @return ResourceInterface
     */
    private function buildResponseFromRaw(array $data, $spaceName = null, AssetDecoratorInterface $assetDecorator = null, $locale = null)
    {
